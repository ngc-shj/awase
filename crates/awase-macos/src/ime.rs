...
mod imp {
    ...
    ///
    /// フィールド計測（2026-09-05〜09-10、通常使用 6 日、ON 切替 n=1253）:
    /// p50 43ms / p90 162 / p99 208 / **max 240ms**。max は計測のたびに
    /// 191 → 208 → 240 と伸びており、300ms とのマージンは残り 60ms。超過は
    /// 6 日で 18 回（全て張り直しで回復、代償は ~0.4 秒の待ち）。この値を
    /// 触るときは裾の伸びを前提にすること。
    ///
    ...
    /// 入力消失とも 0 件。直接測定は依然不可能だが、50ms は実地で保っている。
